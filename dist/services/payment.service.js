...
class PaymentService {
    ...
    async createPayment(uow, body) {
        ...
        const id = (0, generateId_1.generateIdByFormat)("OD", 6, create.id);
     ...
     */
    async updatePayment(uow, paymentId, data) {
        const payment = await uow.payment.findById(paymentId);
        if (!payment) {
            throw new Error(`Payment with ID ${paymentId} not found`);
        }
     ...
     */
    async handlePaymentCallback(uow, referenceNumber, status, additionalData) {
        ...
        // Cập nhật trạng thái
        const [affectedCount, updatedPayments] = await uow.payment.updateStatus(payment.id, status);
        if (affectedCount === 0) {
            throw new Error(`Failed to update payment status`);
        }
        const updatedPayment = updatedPayments[0];
        // Nếu thanh toán thành công, cập nhật trạng thái đơn hàng
        if (status === transaction_1.TransactionStatus.Completed && updatedPayment) {
            await this.handleSuccessfulPayment(uow, updatedPayment);
        }
        else if (status === transaction_1.TransactionStatus.Failed && updatedPayment) {
     ...
     */
    async handleSuccessfulPayment(uow, payment) {
     ...
     */
    async handleFailedPayment(uow, payment) {
     ...
     */
    async deletePayment(uow, paymentId) {
        const payment = await uow.payment.findById(paymentId);
        if (!payment) {
            throw new Error(`Payment with ID ${paymentId} not found`);
        }
        // Chỉ cho phép xóa payment có status là pending hoặc failed
        if (payment.status !== transaction_1.TransactionStatus.Pending && payment.status !== transaction_1.TransactionStatus.Failed) {
