...
const cartCoupon_service_1 = require("./cartCoupon.service");
...
class CartService {
    ...
    async getOrCreateCart(uow, userId) {
        ...
        const appliedCoupons = cart.cartCoupons ?? [];
        const { discount_amount, coupon_details } = cartCouponService.calculateDiscount(summary.total_price, appliedCoupons);
     ...
     */
    async addItemToCart(uow, userId, variantId, quantity) {
        ...
        if (existingItem) {
            ...
            await uow.cartItem.update(existingItem.id, {
     ...
     */
    async removeItemFromCart(uow, cartItemId) {
    ...
    async syncCartSummary(uow, cartId) {
